...
\begin{document}
    ...
    Если функция не является непрерывной, при этом выполняется п.1, то
    ...
    \begin{minipage}{0.49\linewidth}
        ...
        \captionof{figure}{Устранимые разрывы}

        \includegraphics[width=\linewidth]{11_1_3_3.png}
        \captionof{figure}{Неустранимые разрывы}
    \end{minipage}
    ...
    \begin{enumerate}
        ...
        \( \forall \varepsilon > 0\ \exists \delta:\ \abs{x - a} < \delta\quad \abs{f(x) - f(a)} = \abs{c - c} = 0 < \varepsilon \)

        \item \(y=x\) непрерывна \(\forall x\)
        
        \(\forall \varepsilon > 0\ \exists \delta:\ \abs{x - a} < \delta\quad \abs{f(x) - f(a)} = \abs{x - a} < \delta = \varepsilon\)
